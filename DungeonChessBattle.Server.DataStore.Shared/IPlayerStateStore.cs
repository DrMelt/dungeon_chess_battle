...
public interface IPlayerStateStore {
    ...
    void SetRoomHost(RoomId roomId, string hostName);

    /// <summary>登记玩家为房间准备阶段成员，默认未准备，并记录连接归属。</summary>
    void RegisterRoomPlayer(RoomId roomId, string playerName, string playerId, string connectionId);

    /// <summary>登记房间内玩家的 playerId，用于战斗启动时注册白名单。</summary>
    void RegisterRoomPlayerId(RoomId roomId, string playerName, string playerId);

    /// <summary>获取房间内所有玩家的玩家名到 playerId 映射快照。</summary>
    Dictionary<string, string> GetRoomPlayerIds(RoomId roomId);

    /// <summary>设置房间内玩家准备状态，仅限非房主，房主身份不参与准备判定。</summary>
    /// <remarks>未选择角色时拒绝准备，返回是否成功；取消准备不校验角色。</remarks>
    bool TrySetPlayerReady(RoomId roomId, string playerName, bool ready);

    /// <summary>判断房间内指定玩家是否已准备。</summary>
    bool IsPlayerReady(RoomId roomId, string playerName);

    /// <summary>判断房间内所有玩家（含房主）是否都已选择角色。</summary>
    bool AreAllPlayersUnitSelected(RoomId roomId);

    /// <summary>判断房间内除房主外的所有玩家是否都已准备。无其他成员时视为已满足。</summary>
    bool IsAllOthersReady(RoomId roomId);

    /// <summary>获取房间准备状态快照。</summary>
    /// <remarks>副本键原样取自房间配置，未选定为空串；副本键的解析归调用方经内容视图完成，Store 不兜底内容默认值。</remarks>
    RoomStateSnapshot GetRoomState(RoomId roomId);

    /// <summary>判断指定连接是否为指定房间的房主，基于连接归属表。</summary>
    bool IsConnectionRoomHost(string connectionId, RoomId roomId);

    /// <summary>判断指定连接是否为指定房间的成员，基于连接归属表。</summary>
    bool IsConnectionInRoom(string connectionId, RoomId roomId);

    /// <summary>解析指定连接所属的房间 ID，基于连接归属表；未归属任何房间时返回 <see cref="RoomId.None"/>。</summary>
    RoomId GetRoomIdForConnection(string connectionId);
    ...
    /// <summary>移除房间内玩家，连接断开或主动离开清理，返回所属房间 ID；玩家未登记或房间因此被删除时返回 <see cref="RoomId.None"/>。</summary>
    /// <remarks>
    ...
    /// </remarks>
    RoomId RemovePlayerByConnection(string connectionId);
    ...
    bool AddPrepareUnit(RoomId roomId, string unitConfigKey, string campOptionKey,
    ...
    bool RemovePrepareUnit(RoomId roomId, string unitConfigKey, string ownerName);

    /// <summary>获取准备阶段单位列表。</summary>
    IReadOnlyList<UnitSelection> GetPrepareUnits(RoomId roomId);
}
