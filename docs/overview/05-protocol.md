...
- **字段长度约束**：`EntityConstants` 定义玩家名（16）、单位配置键（32）上限与默认副本键，服务端校验与客户端 UI 限制共享同一常量。
- **进程间契约**：`ServerProcessEnv` 定义跨进程环境变量名（密码/父 PID/可执行路径/构建配置），客户端 `ServerProcessHost` 写入、服务器入口与父进程看护读取。
