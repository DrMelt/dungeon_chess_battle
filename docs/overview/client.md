...
- 持有 `LobbyClient`（SignalR）与 `RoomBattleClient`（LiteNetLib + LES）两个持久实例，经 `IClientConnectionFactory` 创建——该接口是传输类型的唯一实例化点，实例只存在于门面内部。
- 对外一律给抽象：房间链路给 `IClientBattleSession` 契约与 `RoomNetworkStatus` 快照，大厅链路给 `Request*` 方法。门面公开签名不含传输类型，连接发起权力由门面独占。
- 主线程驱动：LiteNetLib `NetManager` 非线程安全，SignalR 后台线程回调一律入队 `_mainThreadActions`，由 Godot 主线程每帧 `Update` 消费后再驱动两端网络轮询。
- 约束：所有对房间客户端的操作都收敛主线程。
