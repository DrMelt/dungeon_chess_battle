...
- 房间配置、房间密码、房主名、准备状态（房间 → 玩家名 → 是否就绪）、连接归属（connectionId → 房间+玩家名）、登录会话（connectionId → 登录名）、会话凭证（凭证 → 登录名，另登记连接 → 凭证以供撤销）、玩家记录注册表（登录名 → 主键）、playerId 映射（房间 → 玩家名 → playerId）、准备单位列表。
- 回放归档（roomId → 字节流 + 参与者记录主键）、玩家记录索引（记录主键 → 房间列表）、归档顺序（最近 256 场，超出按归档自身的参与者反向清索引）。摘要不入库，读侧从归档元数据块现取。
...
- `PlayerIdentityResolver` 适配器组合"凭证 → 登录名 → 主键"两步查表实现 `IPlayerIdentityResolver`；凭证无效或所属连接已登出时返回 null 且不登记记录，避免匿名凭证凭空建主键。
- 会话凭证与玩家记录主键都只在进程内有效：服务端重启后同名玩家会派生新主键，旧主键只残留在客户端本地副本里，不再可比。
