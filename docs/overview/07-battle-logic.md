...
这份步序正是回放逻辑修订号所背的东西：步骤顺序、事件产生次序与第 9 步的作废口径任一变动，都要递增 `BattleLogicRevision.Value`，否则旧录像会在同一份输入下重跑出不同结果，而双重门控看不出差别。内容与布局侧的变化不走这里，走 `GameConfigDB.DataRevision`。
