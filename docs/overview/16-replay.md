...
- 每帧顺序与服务端 `BattleLoop` 钩子同形：门面 `PrepareTick`（AI 决策 → 在架施法重试）→ 注入本帧记录 → `Tick`。注入内部按施法 → 移动 → 聚焦，与服务端落点同序；两者都只登记意图，裁定在 `Tick` 内单点完成，故注入先后不再影响结果。施法记录经同一输入门面的排队器接管，排队后的落地时刻一并复现；`Accepted=false` 的记录直接跳过，以服务端投递结论为准。
