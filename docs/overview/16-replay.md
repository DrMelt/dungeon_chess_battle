...
- 每帧顺序与服务端 `BattleLoop` 钩子同形：门面 `PrepareTick`（AI 决策 → 在架施法重试）→ 注入本帧记录 → `Tick`。记录条目经 `ReplayCommands` 还原为玩家命令，交与在线同一个门面提交；注入内部按施法 → 移动 → 聚焦，与服务端落点同序，移动与施法都只登记意图、裁定在 `Tick` 内单点完成，故注入先后不影响结果。施法排队后的落地时刻一并复现；`Accepted=false` 的记录直接跳过，以服务端结论为准，接管后仍被门内规则拒绝的（如聚焦目标已死亡）同样落空。
- 确定性：AI/伤害/移动均为纯函数无随机；重建顺序与 ID 对齐保证记录中的单位引用有效。
- 拖动：SeekTo 目标帧早于当前帧时重建战斗世界并从首帧快进，重建同时清空预输入缓冲——在架意图持旧单位引用。
