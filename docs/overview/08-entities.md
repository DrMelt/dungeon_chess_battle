...
- `BattleRoomEntity`：房间级战斗状态权威载体，实现 `IBattleRoom`。承载阶段、是否结束、开始时刻与副本键；禁止在 OnConstructed 重置同步字段（客户端先应用初始同步再执行 OnConstructed，重置会丢字段）。
- `UnitPawn`：单位 SyncVar 载体，实现 `IBattleUnit`。生命、读条、冷却截止 tick、Buff/仇恨列表、阵营、死亡状态等均为网络同步字段；技能定义、智能决策器、移动管线为装配期本地写入不参与同步。
...
- `IBattleUnit` 适配层把 SyncVar/SyncList 映射为领域读写通道：`Snapshot` 组装只读结算快照、冷却与 Buff 经 `SyncTickHelper` 在服务端写截止 tick、客户端用插值 ServerTick 推算剩余。
- `IBattleRoom` 适配层只做值映射；写成员（ProjectBattleStarted/Ended）仅服务端调用。
...
- `ReplayModels`：`ReplayRecordHeader` / `ReplayPlayerInfo` / `MoveInputRecord` / `CastSkillRecord` / `FocusTargetRecord` / `ReplayRecordSnapshot`，基于 MessagePack 显式 Key 索引，字段重命名不破坏兼容。
- `ReplayRecordCoder` 编解码唯一权威，`ReplayFormatVersion` 门控版本。格式不依赖网络栈，服务端导出与客户端下载解析共用。
