...
- `ReplayCommands` 是玩家命令与记录条目的双向映射唯一权威：载荷拆分只有一份实现，录制端与重放端无从各写一遍。键的换算不在其中——命令持 `UnitId`（`SourceUnitId`/`TargetUnitId`），条目持玩家表序号（`ReplayPlayerInfo` 下标）与 `ushort` 的 `TargetNetId`/`SkillKey`，两端各自反查；命令与条目之间的 ID 升降级只在本类这六个映射里发生。
- `Header.FirstEnemyNetId` 是重放端重建敌方单位 ID 的起点，与录制端条目里的 `TargetNetId` 同一数轴；它由"准备期玩家单位在首帧连续建完"这一前提推出，前提被破坏时两端静默错位，不报错。
