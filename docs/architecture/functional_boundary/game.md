...
- 经唯一入口读取技能/Buff/副本展示资源：表由展示装配填充，主工程不持有对应 `.tres`；工程内 `res://` 路径只由引擎预置资源登记持有。
- mod 装配编排：扫描后交 `GameConfig` 数据装配，把引擎侧注册、资源包挂载与条目落地三个委托交进展示装配；无展示的条目补占位资源。
