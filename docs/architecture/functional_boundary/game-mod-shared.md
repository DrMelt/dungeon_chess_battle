...
展示接口层：宿主实现、mod 调用的全部口——条目数据读写面与装配上下文，以及 mod 实现、宿主调用的表现接口。只依赖 Game.Shared，不依赖 Game.Mod.Interface。

## 职责

- 条目数据读面：四类条目展示数据按内容键读，mod 与宿主 UI 查同一张表。场景模板与图标纹理随条目数据以对象携带，mod 引自己包内的资源。
- 条目数据写面：注册四类条目展示数据，同键后注册覆盖且未声明字段沿用被覆盖者。
...
- Game.Shared，内容身份键经该链出自 Battle.Shared。引擎引用随该链由引擎 SDK 提供，与主工程同版本；本库不定义引擎对象子类，不产生脚本注册需求。
