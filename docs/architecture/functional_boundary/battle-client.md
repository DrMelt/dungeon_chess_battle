...
房间战斗客户端，LiteNetLib + LiteEntitySystem 传输。管理 LES 实体，把服务端实体事件映射为 UI 可消费的接口事件。

## 职责

- 管理网络实体生命周期与事件派发，经 `IClientBattleSession` 契约暴露给上层。
- 保存当前房间会话的战斗事件日志，只读暴露给 UI。
- 上送施放、聚焦与移动请求。
- 把服务端下行的同步字段读数回填进本地领域单位，作为展示与判定的唯一取数源。
