...
- 网络实体：`BattleRoomEntity`、`UnitPawn`、`UnitController`；`BattleRoomEntity` 实现 `IBattleRoom` 房间级战斗状态权威载体契约并承载整帧战斗事件日志 RPC，`UnitPawn` 实现 `IBattleUnit` 并承载服务端权威战斗状态 `UnitCombatState`，不参与网络同步。
- 类型注册表与自定义 SyncVar 字段类型注册，LES 日志转接。
- 协议载荷：可靠请求、同步数据载荷与战斗事件日志编解码 `SyncBattleEvent`/`BattleEventCoder`。
- 移动输入流注入与可靠请求订阅。
