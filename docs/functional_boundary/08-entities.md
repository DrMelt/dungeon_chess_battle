...
- 网络实体：`BattleRoomEntity`、`UnitPawn`、`UnitController`；`BattleRoomEntity` 实现 `IBattleRoom` 房间级战斗状态权威载体契约，`UnitPawn` 实现 `IBattleUnit` 并承载服务端权威战斗状态 `UnitCombatState`，不参与网络同步。
