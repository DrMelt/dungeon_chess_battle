...
- 经 `ResourceTables` 唯一入口读取技能/Buff/副本展示资源；工程内 `res://` 路径只由 `ResourceTables` 与 `BuiltinDisplayAssets` 持有。
- mod 装配编排：扫描后交 `GameConfig` 做数据装配，把「内置展示注册」与「mod 条目落地成资源」两个委托交进 `ModAssets.Assemble`，展示装配内部次序由 `Game.Mod.Manager` 保证。被 mod 声明过的条目以内置资源为模板复制后改写，内容里有的条目没展示时补占位资源。
