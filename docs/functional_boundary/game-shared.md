...
mod 与宿主共用的展示形状层：注册器收发的展示数据、宿主登记的内置资源名。纯形状，零项目引用，不含接口与实现。

## 职责

- 四类条目展示数据 `SkillDisplay` / `BuffDisplay` / `UnitDisplay` / `DungeonDisplay`：未声明的字符串成员为空串、资源成员为 null，回退值由消费方决定；`UnitDisplay` 额外携带模型场景与主体配色，未配置回落共享模板。
- 内置资源名 `DisplayAssetIds`：宿主登记，mod 按名引用特效、范围提示与环境场景，不必硬编码字符串；`res://` 路径映射留在主工程 `BuiltinDisplayAssets`。
