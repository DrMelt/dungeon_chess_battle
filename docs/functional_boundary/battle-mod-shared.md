...
- 内容注册面 `IModContentRuntime`：把技能/Buff/单位/副本定义对象注册进内容注册表，Buff 以 BuffTypeId 为同步身份。
- 合成口 `IModBootstrapContext`：两面的合成句柄，作入口 `IModEntry.Initialize` 的唯一实参，实现在 Battle.GameConfig。

## 边界外

- 不含 mod 要实现的接口：入口契约 `IModEntry` 在 Battle.Mod.Interface，本库不引用它。
- 不含行为 ID 常量：`BehaviorIds` 归 Battle.Shared。
- 不含实现：`ModBootstrapContext`、`BehaviorCatalog` 与内容注册表实现全在 Battle.GameConfig。
- 不含装载与管理：扫描、清单、依赖排序与 ALC 装载在 Battle.Mod.Manager。
