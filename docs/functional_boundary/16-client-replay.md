...
客户端本地回放引擎，所属分组 Client。纯本地确定性重跑录制输入，零网络依赖。
...
- 展示投影：ReplayProjector 把 BattleUnit 状态投影到 ReplayUnitView 展示模型，回放 UI 只读消费。
- 播放控制：播放/暂停/倍速；拖动经 SeekTo 从首帧确定性快进。

## 不负责

- 不依赖网络与 LES，不含回放下载与本地存储。
- 不实现 UI 场景与入口，展示节点、控制条与回放面板在 Godot 主工程 Replay 命名空间。
