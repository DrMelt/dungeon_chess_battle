...
- 事件编排：`BattleScene` 每帧经 `BattleEventLog` 收集结算产出的事件，处理开始清空、处理中只增追加，`Tick` 之外的施法开始/取消事件经跨帧缓冲汇入下帧日志、非 Running 阶段缓冲一并清空；`HateDispatcher` 帧末统一消费事件流产出仇恨效果，`BattleScene` 按持有者路由到单位仇恨表落账。事件日志仅当帧有效。
