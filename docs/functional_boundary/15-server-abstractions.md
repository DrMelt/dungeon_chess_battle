...
- 不包含录制与重跑逻辑，录制在 Server.Battle.Replay、重跑在 Replay 引擎；不包含 HTTP 端点与网络栈，归 Replay.Protocol 与 Replay.Server。
- 不包含实现：广播、房间生命周期由 Server.Lobby 与 Server.Battle 实现，会话凭证与身份解析由 Server.DataStore 实现。
- 不承担凭证的签发与撤销：那是大厅登录流程与存储层的动作，本端口只解析已签发的凭证。
- 不覆盖大厅自身的鉴权：Hub 上的业务身份仍是登录时自报的玩家名，服务器对外暴露前需先加固登录。

## 依赖

- 无。零依赖契约库，供 Server.Lobby、Server.Battle、Server.Replay、Server.DataStore 与 Server.Host 共享。
