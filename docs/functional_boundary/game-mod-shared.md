...
- 加载端口 `IModResourceLoader`：按 `ModAssetKey` 把 mod 包内图片与场景解析成 Godot 对象，契约与寻址键同在本库、实现在 Game.Mod.Manager。寻址键只是包内路径寻址、不是展示形状，不随展示形状进 Game.Shared。
...
- 不含展示数据形状与内置资源名：四个 `*Display`、`DisplayAssetIds` 在 Game.Shared；寻址形状 `ModAssetKey` 随加载端口在本库。
