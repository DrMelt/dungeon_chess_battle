...
- 入口契约 `IModEntry`：ALC 装载后找实现类型，实例化并调 `Initialize(IModBootstrapContext)`。本库只有这一个类型。
...
- 不含 ALC 装载：`ModAssemblyLoader` 与 `ModEntryLoader` 在 Battle.Mod.Manager。
- 不定义包布局：`code/` 等目录名与清单文件名在 `ModLayout`（Battle.Mod.Manager）。
