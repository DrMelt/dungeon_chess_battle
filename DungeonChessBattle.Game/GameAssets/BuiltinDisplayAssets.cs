...
/// <summary>
/// 引擎侧展示注册：把引擎预置场景名与内容单位的外观占位注册进展示注册表。
...
public static class BuiltinDisplayAssets {
    ...
    public static void Register(DisplayRegistry registry) {
        foreach (var (name, path) in EngineScenePaths) {
            ...
            registry.RegisterScene(name, () => GD.Load<PackedScene>(scenePath));
        }
        ...
        foreach (var key in GameContentHost.Registry.Units.Select(unit => unit.ConfigKey))
            registry.RegisterUnit(new UnitDisplay(key, key, "", null, null, null));
    }
}
