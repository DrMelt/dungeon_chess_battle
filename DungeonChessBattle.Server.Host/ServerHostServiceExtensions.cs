...
/// <summary>
/// Server.Host 的 DI 装配扩展：注册存储、内容注册表、大厅/战斗/回放模块与 SignalR，
...
public static class ServerHostServiceExtensions {
    ...
    public static IServiceCollection AddServerHost(this IServiceCollection services,
        ServerConfig config, ILoggerFactory loggerFactory, ContentSetRegistry registry) {
        ...
        // 内容由入口装配一次，这里登记装配产物的只读面供大厅与房间消费
        services.AddSingleton<IContentRegistryView>(registry);
