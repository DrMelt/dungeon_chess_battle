...
/// <summary>
/// 基于进程内字典的回放归档存储实现：房间 ID 主表存字节流，玩家记录主键索引存房间 ID。
...
public sealed class InMemoryReplayStore : IReplayStore {
    ...
    private readonly Dictionary<RoomId, (byte[] Archive, string[] Participants)> _archives = [];
    private readonly Dictionary<string, List<RoomId>> _recordIndex = [];
    private readonly List<RoomId> _order = [];

    /// <inheritdoc />
    public void Add(RoomId roomId, byte[] archive, IReadOnlyList<string> participantRecordIds) {
        lock (_lock) {
    ...
    /// <inheritdoc />
    public IReadOnlyList<RoomId> GetRoomIdsByPlayer(string recordId) {
        lock (_lock) {
            if (!_recordIndex.TryGetValue(recordId, out var rooms))
                return [];
            var result = new List<RoomId>(rooms.Count);
    ...
    /// <inheritdoc />
    public bool TryGetArchive(RoomId roomId, out byte[] archive) {
        lock (_lock) {
