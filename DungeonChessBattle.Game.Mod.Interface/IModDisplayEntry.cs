...
/// <summary>
/// mod 展示代码入口接口：客户端以 AssemblyLoadContext 加载展示 DLL 后，找到实现本接口的类型，
/// 实例化并调用 Initialize，把该 mod 的展示数据注册进展示注册表。
/// 本库是 mod 开发唯一的引用锚点，且只放 mod 要实现的接口：注册表读写口在 <c>Game.Display.Registry</c>、
/// 装配上下文与表现接口在 <c>Game.Mod.Shared</c>、共用的展示数据在 <c>Game.Shared</c>，均经本库的引用链传递可见。
