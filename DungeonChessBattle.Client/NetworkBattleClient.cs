...
public class NetworkBattleClient : IBattleService, INetEventListener
{
    ...
    public GameRoom? GetRoom(string roomId)
    {
        ...
        {
            if (u.Camp.Value == 1) room.UnitsA.Add(new UnitModel { UnitStateName = u.UnitName.Value });
            if (u.Camp.Value == 2) room.UnitsB.Add(new UnitModel { UnitStateName = u.UnitName.Value });
        }
        return room;
    }

    public bool RemoveRoom(string roomId) { _rooms.Remove(roomId); _roomUnits.Remove(roomId); return true; }
    ...
    {
        SendCommand(new { type = "start_battle", roomId });
