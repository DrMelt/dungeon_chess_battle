...
/// <summary>
/// 一次展示装配的产物：展示注册表与 mod 管理根，成员一律只读，管理动作经管理根进行。
...
public sealed class ModAssets(DisplayRegistry registry, ModCatalog catalog, string assemblyFingerprint) {
    ...
    /// 只能留在 Godot 主工程，本库不认识它们。
    /// 装载先于挂载、入口执行后于挂载，展示注册表在装配结束时即最终态。
    /// </remarks>
    ...
    public static ModAssets Assemble(
        ...
        // 装载不执行入口，ALC 由 loaded 持有至入口统一执行
        ...
        // 装载、注册与入口执行三处错误在此统一落日志，条目按归属进 catalog
